...
class Citrix
{
	 ...
	 * Base api url
	 */
	protected $base_url = "https://api.getgo.com";
	...
	*/
	public function get_organizer_key()
	{
		...
		$reponse = json_decode($this->make_request($this->base_url."/G2W/rest/organizers/".$this->organizer_key."/upcomingWebinars?oauth_token=".$this->access_token), true);
	...
	*/
	public function get_access_token($code = null) {
		...
		if($code)
		{
			$reponse = json_decode($this->make_request($this->base_url."/oauth/access_token?grant_type=authorization_code&code={$code}&client_id=".$this->citrix_api_key), true);
	...
	*/
	function auth_citrixonline($redirect_url = '')
    {
		...
		return $this->base_url.'/oauth/authorize?client_id=' . $this->citrix_api_key . '&redirect_uri='. urlencode($redirect_url);
	}
	...
	*/
	function citrixonline_get_list_of_webinars($type = 0)
    {
		...
		if($type>0)
		{
			$reponse = json_decode($this->make_request($this->base_url."/G2W/rest/organizers/".$this->organizer_key."/historicalWebinars?oauth_token=".$this->access_token), true);
	...
	*/
	function get_registrants_of_webinars($webinar_id=false)
    {
		if($webinar_id)
		{
			...
			$return_array = json_decode($this->make_request($this->base_url."/G2W/rest/organizers/".$this->organizer_key."/webinars/{$webinar_id}/registrants?oauth_token=".$this->access_token), true);
	...
	*/
    function citrixonline_create_registrant_of_webinar($webinar_id=false, $data = array(), $all_fields_chk = false)
    {
		if($webinar_id and isset($data['first_name']) and isset($data['last_name']) and isset($data['email']))
		{
			...
			$reponse = json_decode($this->make_request($this->base_url."/G2W/rest/organizers/".$this->organizer_key."/webinars/{$webinar_id}/registrants", $params), true);
	...
	*/
    function citrixonline_delete_registrant_of_webinar($webinar_id=false, $registrant_id=false)
    {
		if($webinar_id && $registrant_id)
		{
			...
			return $this->make_request($this->base_url."/G2W/rest/organizers/".$this->organizer_key."/webinars/{$webinar_id}/registrants/{$registrant_id}", $params);
		}
